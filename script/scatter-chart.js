window.createScatterChart = function() {
    fetch('./json/sleepDiffStats.json')
        ...
        .then(jsonData => {
            ...
            const regressionType = 'quadratic';  // or 'linear'
            const params = computeRegression(scatterPoints, regressionType);

            // Generate line points for the regression line
            const bestFitPoints = generateRegressionPoints(params, scatterPoints, 80, regressionType);

            // Build Chart.js data object
            const data = {
                datasets: [
                    {
                        label: 'Voice Notes vs Sleep Difficulty',
                        data: scatterPoints,
                        showLine: false,
                    ...
                    options: {
                        ...
                        scales: {
                            ...
                            y: {
                                title: {
                                    display: true,
                                    text: 'Sleep Difficulty Change (1 Month)',
                                    color: '#fff',
                                    font: {
                                        size: 14,
                                        weight: 'bold'
